...
def test_apply_is_deterministic_and_imputes_all_nan():
    ...
    t = fit_multimodal_transform(mods, fm, pca_dims=[0.8, 0.8],
                                 post_pca=0.8, pca_method="plain")
...
def test_leakage_free_excluded_rows_do_not_shape_transform():
    ...
    t1 = fit_multimodal_transform([m.clone() for m in mods], fm,
                                  pca_dims=[0.8, 0.8], post_pca=0.8,
                                  pca_method="plain")

    perturbed = [m.clone() for m in mods]
    for m in perturbed:
        m[:20] = 1e4                                   # extreme values on excluded rows
    t2 = fit_multimodal_transform(perturbed, fm, pca_dims=[0.8, 0.8],
                                  post_pca=0.8, pca_method="plain")
    ...
    def _same_V(a, b, label):
        if a is None or b is None:
            ...
            assert torch.allclose(a, b), f"{label}: PCA basis leaked from excluded rows"
...
def test_post_pca_none_skips_post_normalize():
    ...
    fm = torch.ones(mods[0].shape[0], dtype=torch.bool)
    t = fit_multimodal_transform(mods, fm, pca_dims=[0.8, 0.8],
                                 post_pca=None, pca_method="plain")
...
def test_robust_pca_returns_finite_projection():
    ...
    res = _fit_pca_V(X, 0.8, method="robust")          # falls back to median-SVD if no robpy
    if res is not None:
...
def test_fold_fit_mask_excludes_test_only_genes():
    with tempfile.TemporaryDirectory() as d:
        ...
        dm = SLDataManager(embeddings_paths=[p1, p2], sl_pairs_path=slp,
                           seed=42, preprocessing_fit_scope="train")

        # Hand-crafted fold: train touches genes {0,1}, test touches {3,4}.
        fold = {
            "fold": 0,
            "train": {"pairs": np.array([[0, 1]], dtype=np.int64)},
            "test": {"pairs": np.array([[3, 4]], dtype=np.int64)},
        }
        mask = dm._fold_fit_mask(fold)
        assert mask.dtype == torch.bool and mask.numel() == 5
        assert bool(mask[0]) and bool(mask[1]) and bool(mask[2]), "train/other genes kept"
        assert not bool(mask[3]) and not bool(mask[4]), "test-only genes must be excluded"

        dm.preprocessing_fit_scope = "all"
        assert dm._fold_fit_mask(fold).all(), "scope='all' must keep every gene"
...
def test_pair_label_mask_invariants():
    ...
    assert lab[K562] == 1 and msk[K562] == 1 and lab[A549] == 0 and msk[A549] == 1

    lab, msk = clv.pair_label_mask("K562", "K562", True, True)   # same-head disagreement
    assert msk[K562] == 0 and msk.sum() == 0

    lab, msk = clv.pair_label_mask("", "A549", True, True)       # MISSING SL -> OTHER
    assert lab[OTHER] == 1 and msk[OTHER] == 1

    lab, msk = clv.pair_label_mask("RPE1", "", True, False)      # rare line -> OTHER
    assert lab[OTHER] == 1 and msk[OTHER] == 1
...
def test_per_head_loader_wrapper_and_freeze():
    ...
    from eval_finetuning import (
        load_multicell_model, _HeadSelect, FinetunedSL, set_trainable,
    )

    D, cell_dim = 12, 8
    H = clv.NUM_HEADS
    mc = SiameseSLMultiCell(input_dim=D, num_heads=H, encoder_dims=[6, 6, 6],
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        torch.save({"model_state_dict": mc.state_dict(),
                    "config": {"pd_epsilon": 0.001}}, ckpt)
