...
from siamese_esm import SiameseSL, set_seed
...
def split_pairs(n: int, train_frac: float, val_frac: float,
    ...
    """Deterministic 80/10-of-80/20 pair split. Shared across folds.
...
def parse_datasets_spec(specs: List[str]) -> List[Dict]:
    ...
    for s in specs:
        parts = s.split(":")
        if len(parts) != 5:
            raise ValueError(f"DATASETS row must have 5 colon-separated "
                             f"fields (name:file:column:sign:format); got: {s}")
        name, filepath, column, sign_str, _fmt = parts
        out.append({
            "name": name,
            "file": filepath,
            "column": column,
...
def run_for_dataset(
    ...
    for k, ckpt in enumerate(checkpoint_paths):
        ...
        base = load_model(str(ckpt), model_type="siamese", device=device)
        # Sanity: the encoder's input_bias is (input_dim,) — cheap to check
        # that the applied transform's output dim matches the weights.
        expected_dim = int(base.encoder.input_bias.shape[0])
        ...
        for mode in args.ft_modes:
            ...
            base_ft = load_model(str(ckpt), model_type="siamese", device=device)
...
def main() -> int:
    ...
    p.add_argument("--datasets", nargs="+", required=True,
                   help="Spec rows 'name:file:column:sign:format'.")
    ...
    out_dir = Path(args.out_dir) if args.out_dir else (model_dir / "eval_finetuning")
