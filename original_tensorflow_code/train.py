...
adj_orig = sp.csr_matrix(
    (np.ones(len(pos_edge)), (pos_edge[:, 0], pos_edge[:, 1])),
    shape=(6375, 6375))
...
for train_pos, test_pos in pos_edge_kf:
    ...
    # build training set
    mask_index = np.vstack(
        (pos_edge[test_pos], neg_edge[test_neg[:len(test_pos)]]))
    ...
    placeholders = {
        'support': [
            tf.sparse_placeholder(tf.float32, name='adj_{}'.format(_))
            for _ in range(num_supports)
        ],
        'features':
        tf.sparse_placeholder(tf.float32, name='features'),
        'adj_orig':
        tf.sparse_placeholder(tf.float32, name='adj_orig'),
        'dropout':
        tf.placeholder_with_default(0., shape=(), name='dropout'),
    }

    # Create model
    model = SLMGAE(placeholders,
                   num_features,
                   features_nonzero,
                   num_nodes,
                   num_supports - 1,
    ...
    with tf.name_scope('optimizer'):
        opt = Optimizer(supp=model.support_recs,
                        main=model.main_rec,
    ...
    feed_dict = construct_feed_dict(supports, features, adj_label,
                                    placeholders)
...
log("Average metrics over pairs:\n" + " time_mean:%.5f, time_sdv:%.5f\n" %
    (m4, sdv4) + " auc_mean:%.5f, auc_sdv:%.5f\n" % (m1, sdv1) +
