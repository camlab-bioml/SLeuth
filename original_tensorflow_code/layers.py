...
class AttentionRec():
    ...
    def __init__(self,
        ...
        with tf.variable_scope(self.name + '_attW'):
            self.attweights = tf.get_variable(
                "attWeights",
                [self.num_support, self.num_nodes, self.num_nodes],
                initializer=tf.random_uniform_initializer(minval=0.9,
                                                          maxval=1.1))
...
class InnerProductDecoder():
    ...
    def __init__(self, output_dim, name, dropout=0., act=tf.nn.sigmoid):
        ...
        self.vars = {}
        with tf.variable_scope(self.name + '_vars'):
            self.vars['weights'] = weight_variable_glorot(output_dim,
                                                          output_dim,
                                                          name='weights')
        self.dropout = dropout
        self.act = act
