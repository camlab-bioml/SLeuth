    ...
    def __init__(self, supp, main, preds, labels, num_nodes, num_edges, index):
        ...
        with tf.control_dependencies(update_ops):
            ...
            for viewRec in supp:
                ...
                self.loss_supp += tf.compat.v1.keras.losses.MSE(
                    labels_sub, viewRec_sub)

            self.loss_main = tf.compat.v1.keras.losses.MSE(
                labels_sub, main_sub)

            self.loss_preds = tf.compat.v1.keras.losses.MSE(
                labels_sub, preds_sub)
