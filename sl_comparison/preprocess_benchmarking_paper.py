# Code downloaded from https://raw.githubusercontent.com/JieZheng-ShanghaiTech/SL_benchmark/refs/heads/main/src/preprocess.py
...
class ChecktoSave:
    ...
    def __init__(self, kfold):
        ...
        self.best_ranking = np.asarray([-np.inf] * 12 * kfold).reshape(
            (kfold, 12))
    ...
    def get_all_indep_test_metrics(self):
        all_test_metrics = np.hstack(
            [self.indep_test_classify, self.indep_test_ranking])
        all_valid_metrics = np.hstack([self.best_classify,
                                       self.best_ranking])  # valid
    ...
    def get_all_metrics(self):
        ...
        all_train_metrics = np.hstack(
            [self.best_train_classify, self.best_train_ranking])
...
# Commented out - this is injected by data_split.py at runtime (line 66)
...
human_sl_pairs_df = None  # Will be injected by data_split.py

# Commented out - these files don't exist in our data structure
...
meta_table = None  # Not used by cv1/cv2/cv3
all_uni_id_cut = None  # Not used by cv1/cv2/cv3
...
def get_id_map():
    ...
    id_mapping = dict(
        zip(list(meta_table["symbol"]), list(meta_table["unified_id"])))
    num_node = len(
        set(human_sl_pairs_df["unified_id_A"])
        | set(human_sl_pairs_df["unified_id_B"]))
...
def cv1_all(
    ...
    pos_position = np.asarray(pos_position[["unified_id_A",
                                            "unified_id_B"]].values,
                              dtype=int)
    for i in range(len(pos_position)):
        if pos_position[i, 0] > pos_position[i, 1]:
            pos_position[i, 0], pos_position[i, 1] = (
                pos_position[i, 1],
                pos_position[i, 0],
            )
    if negative_strategy == "All_Random":
        ...
        neg_position = sp.csr_matrix(
            (np.ones(neg_position.shape[0]),
             (neg_position[:, 0], neg_position[:, 1])),
            shape=(num_node, num_node),
        )
        neg_position = np.asarray((neg_position + neg_position.T).toarray(),
                                  dtype="bool")
    ...
    if negative_strategy == "Random" or negative_strategy == "All":
        ...
        for ind_a in range(num_node):
            for ind_b in range(num_node):
                if (ind_a, ind_b) in sl_pos_set or (ind_b,
                                                    ind_a) in sl_pos_set:
                    continue
                if ind_a < ind_b:
                    pair = (ind_a, ind_b)
        ...
        if xtimes == 1000:
            ...
            neg_position = neg_position[random.sample(
                init_neg_index, int(training_size * xtimes))]
        ...
        if ex_compt:
            ...
            new_neg_df = new_neg_df[
                new_neg_df[0].isin(all_uni_id_cut.keys())
                & new_neg_df[1].isin(all_uni_id_cut.keys())]
...
def cv2(
    ...
    SL_pos_sparse = sp.csr_matrix(
        (np.ones(len(pos_position)), (pos_position[:, 0], pos_position[:, 1])),
        shape=(num_node, num_node),
        dtype="bool",
    )
    SL_pos_sparse = SL_pos_sparse + SL_pos_sparse.T
    gene_a_pos_sparse = sp.csr_matrix(
        (pos_position[:, 0], (pos_position[:, 0], pos_position[:, 1])),
        shape=(num_node, num_node),
    )
    gene_b_pos_sparse = sp.csr_matrix(
        (pos_position[:, 1], (pos_position[:, 0], pos_position[:, 1])),
        shape=(num_node, num_node),
    )

    SL_neg_sparse = sp.csr_matrix(
        (np.ones(len(neg_position)), (neg_position[:, 0], neg_position[:, 1])),
        shape=(num_node, num_node),
        dtype="bool",
    )
    SL_neg_sparse = SL_neg_sparse + SL_neg_sparse.T
    gene_a_neg_sparse = sp.csr_matrix(
        (neg_position[:, 0], (neg_position[:, 0], neg_position[:, 1])),
        shape=(num_node, num_node),
    )
    gene_b_neg_sparse = sp.csr_matrix(
        (neg_position[:, 1], (neg_position[:, 0], neg_position[:, 1])),
        shape=(num_node, num_node),
    )
...
def cv3(
    ...
    for train_index, test_index in kf.split(index_pos):
        ...
        train_pos_sl_id = cv3_division(train_ind_set, SL_pos_sparse,
                                       gene_a_pos_sparse, gene_b_pos_sparse,
                                       num_node)
        train_pos_spm = sparse_matrix_from_adj(train_pos_sl_id, num_node)
        train_pos_kfold.append(train_pos_sl_id)
        graph_train_pos_kfold.append(train_pos_spm)

        test_pos_sl_id = cv3_division(test_ind_set, SL_pos_sparse,
                                      gene_a_pos_sparse, gene_b_pos_sparse,
                                      num_node)
        test_pos_spm = sparse_matrix_from_adj(test_pos_sl_id, num_node)
        test_pos_kfold.append(test_pos_sl_id)
...
def get_kfold_data_pos_neg(
    ...
    exp_data_paths = [
        "../data/preprocessed_data/one_time_neg_index_exp.npy",
    ...
    dep_data_paths = [
        "../data/preprocessed_data/one_time_neg_index_dep.npy",
    ...
    if ex_compt:
        exp_data_paths = [
            "../data/preprocessed_data/wo_compt/one_time_neg_index_exp_wo_compt.npy",
        ...
        dep_data_paths = [
            "../data/preprocessed_data/wo_compt/one_time_neg_index_dep_wo_compt.npy",
        ...
        exp_score_data_path = (
            "../data/preprocessed_data/wo_compt/sorted_neg_ids_scores_exp_wo_compt.npy"
        )
        dep_score_data_path = (
            "../data/preprocessed_data/wo_compt/sorted_neg_ids_scores_dep_wo_compt.npy"
        )
    ...
    if negative_strategy == "Exp":
        if division_strategy == "CV1":
            if os.path.exists(f"{train_data_path}CV1_{xtimes}_Exp.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV1_{xtimes}_Exp.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv1(
                    kfold,
                    num_node,
                    train_rat,
                    valid_rat,
                ...
                np.save(f"{train_data_path}CV1_{xtimes}_Exp.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV2":
            if os.path.exists(f"{train_data_path}CV2_{xtimes}_Exp.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV2_{xtimes}_Exp.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv2(
                    kfold,
                    num_node,
                    train_rat,
                    valid_rat,
                ...
                np.save(f"{train_data_path}CV2_{xtimes}_Exp.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV3":
            if os.path.exists(f"{train_data_path}CV3_{xtimes}_Exp.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV3_{xtimes}_Exp.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv3(
                    kfold,
                    num_node,
                    train_rat,
                    valid_rat,
                ...
                np.save(f"{train_data_path}CV3_{xtimes}_Exp.npy",
                        [pos_samples, neg_samples])
        else:
            print(
                'Please select data division strategy. (one of ["CV1","CV2","CV3"])'
            )
    elif negative_strategy == "Dep":
        if division_strategy == "CV1":
            if os.path.exists(f"{train_data_path}CV1_{xtimes}_Dep.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV1_{xtimes}_Dep.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv1(
                    kfold,
                ...
                np.save(f"{train_data_path}CV1_{xtimes}_Dep.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV2":
            if os.path.exists(f"{train_data_path}CV2_{xtimes}_Dep.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV2_{xtimes}_Dep.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv2(
                    kfold,
                ...
                np.save(f"{train_data_path}CV2_{xtimes}_Dep.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV3":
            if os.path.exists(f"{train_data_path}CV3_{xtimes}_Dep.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV3_{xtimes}_Dep.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv3(
                    kfold,
                ...
                np.save(f"{train_data_path}CV3_{xtimes}_Dep.npy",
                        [pos_samples, neg_samples])
        else:
            print(
                'Please select data division strategy. (one of ["CV1","CV2","CV3"])'
        ...
        if division_strategy == "CV1":
            if os.path.exists(f"{train_data_path}CV1_{xtimes}.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV1_{xtimes}.npy", allow_pickle=True)
            else:
                pos_samples, neg_samples = cv1(
                    kfold,
                    num_node,
                ...
                np.save(f"{train_data_path}CV1_{xtimes}.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV2":
            if os.path.exists(f"{train_data_path}CV2_{xtimes}.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV2_{xtimes}.npy", allow_pickle=True)
            else:
                pos_samples, neg_samples = cv2(
                    kfold,
                    num_node,
                ...
                np.save(f"{train_data_path}CV2_{xtimes}.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV3":
            if os.path.exists(f"{train_data_path}CV3_{xtimes}.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV3_{xtimes}.npy", allow_pickle=True)
            else:
                pos_samples, neg_samples = cv3(
                    kfold,
                    num_node,
                ...
                np.save(f"{train_data_path}CV3_{xtimes}.npy",
                        [pos_samples, neg_samples])
        else:
            print(
                'Please select data division strategy. (one of ["CV1","CV2","CV3"])'
        ...
        if division_strategy == "CV1":
            if os.path.exists(f"{train_data_path}CV1_1000.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV1_1000.npy", allow_pickle=True)
            else:
                pos_samples, neg_samples = cv1(
                    kfold,
                    num_node,
                ...
                np.save(f"{train_data_path}CV1_1000.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV2":
            if os.path.exists(f"{train_data_path}CV2_1000.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV2_1000.npy", allow_pickle=True)
            else:
                pos_samples, neg_samples = cv2(
                    kfold,
                    num_node,
                ...
                np.save(f"{train_data_path}CV2_1000.npy",
                        [pos_samples, neg_samples])
        elif division_strategy == "CV3":
            if os.path.exists(f"{train_data_path}CV3_1000.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV3_1000.npy", allow_pickle=True)
            else:
                pos_samples, neg_samples = cv3(
                    kfold,
                    num_node,
                ...
                np.save(f"{train_data_path}CV3_1000.npy",
                        [pos_samples, neg_samples])
        else:
            print(
                'Please select data division strategy. (one of ["CV1","CV2","CV3"])'
        ...
        if division_strategy == "CV1":
            if os.path.exists(f"{train_data_path}CV1_{xtimes}_All_Random.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV1_{xtimes}_All_Random.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv1_all(
                    kfold,
                    num_node,
                    train_rat,
                    valid_rat,
        ...
        if division_strategy == "CV1":
            if os.path.exists(f"{train_data_path}CV1_{xtimes}_All_Exp.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV1_{xtimes}_All_Exp.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv1_all(
                    kfold,
        ...
        if division_strategy == "CV1":
            if os.path.exists(f"{train_data_path}CV1_{xtimes}_All_Dep.npy"):
                pos_samples, neg_samples = np.load(
                    f"{train_data_path}CV1_{xtimes}_All_Dep.npy",
                    allow_pickle=True)
            else:
                pos_samples, neg_samples = cv1_all(
                    kfold,
...
def NDCG(y_pos_index, y_not_seen, topk):
    ...
    return (
        0 if dcg_topk == 0 or idcg_topk == 0 else dcg_topk / idcg_topk,
        hit_topk / len(y_pos_index),
        hit_topk / min(topk, len(y_pos_index)),
    )
...
def cal_metrics(score_mat, pos_index, neg_index, seen_index=None):
    ...
    y_true = np.hstack([np.ones([1, len(pos_s)]),
                        np.zeros([1, len(neg_s)])]).reshape(-1, 1)
    ...
    # + 1e-10: the reference form 2PR/(P+R) divides by zero wherever a PR point
    # has precision == recall == 0. On SLGNN cv1/fold_0 that is 2 of 25,333
    # cells, and the result only avoids propagating because Python's builtin
    # max() compares with `>`, which is False against NaN — so it survives only
    # as long as no NaN lands at index 0. The epsilon removes the NaN outright
    # and shifts the reported maximum by 4.9e-11 (0.3773584906 -> 0.3773584905),
    # i.e. seven orders of magnitude below the four decimals ever published.
    # NOTE: this is the ONE deviation from the vendored SL_benchmark source.
    f1_p = max(2 * precision * recall / (precision + recall + 1e-10))

    score_matrix_ndcg = copy.deepcopy(score_mat)
    score_matrix_ndcg[range(n_gene), range(n_gene)] = 0
    seen_score = -999999
    if seen_index is not None:
        score_matrix_ndcg[seen_index[:, 0], seen_index[:, 1]] = (
            seen_score  # del seen samples
        )
    ...
    ndcg_k, precision_k, recall_k, map_k = (
        np.zeros(3),
        np.zeros(3),
        np.zeros(3),
        np.zeros(3),
    ...
    precision_k[0] = (y_bool_list[:, :10].sum(axis=1) /
                      np.minimum(y_pos_num_list, 10)).mean()
    precision_k[1] = (y_bool_list[:, :20].sum(axis=1) /
                      np.minimum(y_pos_num_list, 20)).mean()
    precision_k[2] = (y_bool_list[:, :50].sum(axis=1) /
                      np.minimum(y_pos_num_list, 50)).mean()
...
def load_kg(neighbor_sample_size):
    ...
    n_entity = len(
        set(kg_wo_sl["unified_id_A"]) | set(kg_wo_sl["unified_id_B"]))
    ...
    if neighbor_sample_size == "slgnn":
        ...
        adj_relation = np.zeros([n_entity, neighbor_sample_size],
                                dtype=np.int64)
...
def load_gcatsl_features():
    ...
    ppi_spm = sp.load_npz(
        "../data/preprocessed_data/ppi_sparse_upper_matrix_without_sl_relation_9845.npz"
    )
    ...
    go_sim = np.load(
        "../data/preprocessed_data/final_gosim_bp_from_r_9845.npy")
    ...
    go_sim_cc = np.load(
        "../data/preprocessed_data/final_gosim_cc_from_r_9845.npy")
