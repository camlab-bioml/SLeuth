#!/bin/bash
#SBATCH --job-name=SLMGAE_main_noESM
...
mkdir -p ../logs
mkdir -p ../outputs/main_without_ESM
mkdir -p ../outputs/main_without_ESM/checkpoints
mkdir -p ../outputs/main_without_ESM/predictions
...
echo "  - Output dir: ../outputs/main_without_ESM"
...
$PYTHON_PATH train_slmgae.py \
	...
	--pos_neg_ratio 1 \
	--seed 123 \
	--output_dir ../outputs/main_without_ESM
...
for fold in 0 1 2 3 4; do
	PRED_FILE="../outputs/main_without_ESM/fold_${fold}_cv1_predictions.csv"
	if [ -f "$PRED_FILE" ]; then
...
if [ -f "../outputs/main_without_ESM/training_summary.json" ]; then
	$PYTHON_PATH -c "
import json
with open('../outputs/main_without_ESM/training_summary.json', 'r') as f:
    log = json.load(f)
    if 'auc_mean' in log:
...
if [ -f "../outputs/main_with_ESM/training_summary.json" ]; then
	echo ""
...
with open('../outputs/main_with_ESM/training_summary.json', 'r') as f:
    esm = json.load(f)
if 'auc_mean' in baseline and 'auc_mean' in esm:
    diff = esm['auc_mean'] - baseline['auc_mean']
    print(f'  Baseline AUC: {baseline[\"auc_mean\"]:.4f}')
    print(f'  ESM AUC: {esm[\"auc_mean\"]:.4f}')
    print(f'  Improvement: {diff:.4f} ({diff/baseline[\"auc_mean\"]*100:.1f}%)')
    "
fi

echo "=================================================="
echo "Completed at: $(date)"
echo "All 6375x6375 prediction matrices saved in: ../outputs/main_without_ESM/"
echo "=================================================="
