#!/bin/bash
#SBATCH --job-name=SLMGAE_CV3_ESM
...
#SBATCH --output=logs/train_CV3_with_ESM_%j.out
#SBATCH --error=logs/train_CV3_with_ESM_%j.err

# Exit on error
set -e

echo "=================================================="
echo "SLMGAE CV3 Training WITH ESM Embeddings"
echo "=================================================="
...
mkdir -p ../logs
mkdir -p ../outputs/CV3_with_ESM
mkdir -p ../outputs/CV3_with_ESM/checkpoints
mkdir -p ../outputs/CV3_with_ESM/predictions
...
echo "Starting CV3 training with ESM embeddings..."
echo "Configuration:"
echo "  - CV Type: CV3 (pair-based cross-validation)"
echo "  - Epochs: 300"
echo "  - Using ESM: YES"
echo "  - Pos:Neg ratio: 1:1"
echo "  - Output dir: ../outputs/CV3_with_ESM"
...
$PYTHON_PATH train_slmgae_with_esm.py \
	...
	--pos_neg_ratio 1 \
	--seed 123 \
	--output_dir ../outputs/CV3_with_ESM
...
for fold in 0 1 2 3 4; do
	PRED_FILE="../outputs/CV3_with_ESM/fold_${fold}_cv3_predictions.csv"
	if [ -f "$PRED_FILE" ]; then
...
if [ -f "../outputs/CV3_with_ESM/training_summary.json" ]; then
	$PYTHON_PATH -c "
import json
with open('../outputs/CV3_with_ESM/training_summary.json', 'r') as f:
    log = json.load(f)
    if 'auc_mean' in log:
...
echo "All 6375x6375 prediction matrices saved in: ../outputs/CV3_with_ESM/"
echo "=================================================="
