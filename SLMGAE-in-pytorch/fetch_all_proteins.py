...
class ComprehensiveProteinFetcher:
    ...
    def process_all_genes(self):
        ...
        The gene list (List_Proteins_in_SL.txt) contains Entrez Gene IDs.
        UniProt/NCBI queries need gene symbols, so we reverse-lookup
        Entrez ID -> symbol via the gene_id_mapping.tsv table.
        """
        print("=" * 60)
        print("Comprehensive Protein Sequence Fetching")
        print("=" * 60)

        # Read gene list (Entrez IDs)
        with open("../data/List_Proteins_in_SL.txt", "r") as f:
            gene_ids = [line.strip() for line in f if line.strip()]

        # Load Entrez -> symbol mapping for UniProt queries
        import csv
        entrez_to_sym = {}
        mapping_path = "../data/gene_id_mapping.tsv"
        if os.path.exists(mapping_path):
            with open(mapping_path) as f:
                reader = csv.DictReader(f, delimiter='\t')
                for row in reader:
                    entrez_to_sym[row['entrez_id']] = row['symbol']
            print(f"Loaded {len(entrez_to_sym)} Entrez->symbol mappings")

        # Map IDs to symbols for sequence fetching
        genes = []
        id_to_sym = {}
        for eid in gene_ids:
            sym = entrez_to_sym.get(eid, eid)  # fallback to ID itself
            genes.append(sym)
            id_to_sym[eid] = sym
        ...
        # Process each gene with progress bar (fetch by symbol)
        ...
        with open("../protein_seq/main_protein_seq.fasta", "w") as f:
            ...
            # Headers use Entrez Gene IDs; sequences fetched by symbol
            for eid, sym in zip(gene_ids, genes):
                seq = sequences[sym]
                f.write(f">{eid}\n{seq}\n")
