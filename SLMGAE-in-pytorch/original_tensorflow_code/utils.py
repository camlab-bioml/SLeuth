...
def load_data(knn=False, nnSize=0):
    ...
    adjs.append(
        load_dense_feature('../data/Human_GOsim.txt', knn=knn, nnSize=nnSize))
    adjs.append(
        load_dense_feature('../data/Human_GOsim_CC.txt',
                           knn=knn,
                           nnSize=nnSize))
...
def load_BC_data():
    ...
    adjs.append(sp.coo_matrix(
        np.loadtxt('../BC_data/F1_F2_pathway_for_train')))
    adjs.append(
        sp.coo_matrix(np.loadtxt('../BC_data/F1_F2_proteincomplex_for_train')))
...
def load_SL_matrix():
    """Load SL adjacency matrix. Gene IDs are NCBI Entrez Gene IDs."""
    print("Loading SL matrix...")

    slMapping, NumNodes = dict(), 0  # Entrez Gene ID -> index
